...
export function calculatePointsUpdate(
  ...
  if (newRewardPoints > 0) {
    ...
    messages.push(
      `Bạn đã nhận được ${newRewardPoints} điểm thưởng cho đơn hàng trên $${threshold}!`
    );
  }

  if (loyaltyPointsEarned > 0) {
    messages.push(
      `Bạn đã nhận được ${loyaltyPointsEarned} điểm thành viên thân thiết khi hoàn thành ${newCompletedOrders} đơn hàng!`
    );
  }
