...
export default function BusinessAccountsAdmin() {
  ...
  const fetchBusinessAccounts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error("Không thể tải danh sách tài khoản doanh nghiệp");
      }
    } catch (error) {
      console.error("Error fetching business accounts:", error);
      toast.error("Lỗi khi tải danh sách tài khoản doanh nghiệp");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApproval = async (accountId: string, approve: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          `Tài khoản doanh nghiệp đã được ${approve ? "phê duyệt" : "từ chối"} thành công`
        );
        fetchBusinessAccounts(); // Làm mới danh sách
      } else {
        const errorData = await response.json();
        toast.error(errorData.error || "Không thể cập nhật tài khoản doanh nghiệp");
      }
    } catch (error) {
      console.error("Error updating business account:", error);
      toast.error("Lỗi khi cập nhật tài khoản doanh nghiệp");
    } finally {
      setProcessing(null);
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Quản lý và phê duyệt các yêu cầu đăng ký tài khoản doanh nghiệp
        </p>
      </div>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              Không có tài khoản doanh nghiệp nào
            </h3>
            <p className="text-gray-500">Không tìm thấy yêu cầu đăng ký tài khoản doanh nghiệp nào.</p>
          </CardContent>
        </Card>
      ) : (
