  ...
  try {
    ...
    if (!userId || !type || !reason?.trim()) {
      return NextResponse.json(
        { success: false, message: "Thiếu các thông tin bắt buộc" },
        { status: 400 }
      );
    }
    ...
    if (currentStatus !== "pending") {
      return NextResponse.json(
        { success: false, message: `Tài khoản ${type} không ở trạng thái chờ duyệt` },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `❌ Đã từ chối tài khoản ${type === "premium" ? "Premium" : "Doanh nghiệp"
        } của ${user.firstName} ${user.lastName}`,
    });
  } catch (error) {
    console.error("Error rejecting account:", error);
    return NextResponse.json(
      { success: false, message: "Không thể từ chối tài khoản này" },
      { status: 500 }
    );
  }
}