  ...
  try {
    ...
    const notifications = [
      ...
      ...recentOrders.map((order: Order) => ({
        id: `order-${order._id}`,
        title: `Đơn hàng mới ${order.orderNumber || `#${order._id.slice(-6)}`}`,
      ...
      ...lowStockProducts.map((product: Product) => ({
        ...
        description: `${product.title} - Chỉ còn lại ${product.stock}`,
        time: "Hôm nay",
        type: "warning",
        icon: "alert-triangle",
      })),
      ...
      {
        id: "system-1",
        title: "Đã hoàn thành sao lưu hàng ngày",
        description: "Tất cả dữ liệu đã được sao lưu thành công",
        time: "2 giờ trước",
        type: "success",
        icon: "check-circle",
      },
...
function getTimeAgo(date: Date): string {
  ...
  return `${diffInDays} ngày${diffInDays > 1 ? "" : ""} trước`;
}