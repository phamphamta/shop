...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!userEmail || !isUserAdmin(userEmail)) {
      return NextResponse.json(
        { error: "Bị từ chối - Yêu cầu quyền quản trị viên" },
        { status: 403 }
      );
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!orderIds || !Array.isArray(orderIds) || orderIds.length === 0) {
      return NextResponse.json(
        { error: "Danh sách ID đơn hàng là bắt buộc" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: `Đã xóa thành công ${successCount} đơn hàng${failureCount > 0 ? `, xóa thất bại ${failureCount} đơn` : ""
          }`,
