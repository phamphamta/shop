...
export async function POST(
  ...
  try {
    ...
    if (order.paymentStatus !== "paid" && order.status !== "paid") {
      return NextResponse.json(
        { error: "Không thể tạo hóa đơn cho đơn hàng chưa thanh toán" },
        { status: 400 }
      );
    }
    ...
    const invoice = await stripe.invoices.create({
      ...
      description: `Hóa đơn cho Đơn hàng ${order.orderNumber}`,
      metadata: {
        orderId: order._id,
