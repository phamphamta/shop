...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Không thể lấy danh sách địa chỉ" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !address || !city || !state || !zip) {
      return NextResponse.json(
        { error: "Họ tên, địa chỉ, thành phố, bang/tỉnh và mã bưu điện là bắt buộc" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Không thể thêm địa chỉ mới" },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!_id) {
      return NextResponse.json(
        { error: "Yêu cầu cung cấp ID địa chỉ" },
        { status: 400 }
      );
    }
    ...
    if (!existingAddress) {
      return NextResponse.json(
        { error: "Không tìm thấy địa chỉ hoặc quyền truy cập bị từ chối" },
        { status: 404 }
      );
    }
