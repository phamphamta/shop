        ...
        <div className="flex items-center">
          {displayProducts.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className={`relative w-8 h-8 rounded-full overflow-hidden border-2 border-white shadow-sm bg-gray-100 ${index > 0 ? "-ml-2" : ""
                  } z-${30 - index * 10}`}
              ...
              <TooltipTrigger asChild>
                <TableRow className="hover:bg-gray-50 h-16">
                  ...
                  <TableCell>
                    {order?.status && (
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${order.paymentStatus === "paid" ||
